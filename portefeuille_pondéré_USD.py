...
def preprocess_data(df, column_name, start_date, fee_rate):
    """
    Prépare les données : format datetime, tri par date, filtre par start_date,
...
weights = {
    ...
    'VL_SP500': 0.20,
    'VL_Nasdaq100': 0.20,   # Nom identique au code architecture
    'VL_Small_Cap': 0.10,
}
...
def simulate_monthly_investment(df, monthly_investments):
    """
    Simule des investissements mensuels (logique ancien code).
    """
    results = {}
...
def calculate_performance(df, results):
    """
    Logique ancien code pour le rendement (ex. 10.59%).
    """
    performance_table = []
