...
files = {
    ...
    # IMPORTANT : correspondance exacte avec le code architecture (Nasdaq-100)
...
# Alignement avec le code architecture (par ex. "Nasdaq-100" au lieu de "Nasdaq")
fees = {
...
def preprocess_data(df, column_name, start_date, fee_rate):
    """
    Prépare les données : format datetime, trie, filtre par start_date, puis applique les frais (journaliers).
    """
...
# Préparer chaque fichier avec les noms de colonnes en adéquation avec le code architecture
...
# On nomme la colonne du Nasdaq comme dans le code architecture : VL_Nasdaq100
...
# IMPORTANT : Même nom de colonne que ci-dessus, donc 'VL_Nasdaq100' et non 'VL_Nasdaq'
weights = {
...
def calculate_portfolio_value(df, weights):
    """
    Calcule la valeur du portefeuille : somme pondérée des VL, normalisée sur 10 000 €.
    """
    portfolio_value = sum(
...
def simulate_monthly_investment(df, monthly_investments):
    """
    Simule des investissements mensuels pour différents montants.
    """
    results = {}
...
def calculate_performance(df, results):
    """
    Calcule le rendement total, le rendement annualisé et la valeur finale après impôts.
    """
    performance_table = []
