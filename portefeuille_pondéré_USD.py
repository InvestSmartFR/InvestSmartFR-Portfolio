...
files = {
    ...
    # On renomme ici "Nasdaq" en "Nasdaq-100" pour être cohérent avec le code architecture
...
# On renomme ici la clé "Nasdaq" → "Nasdaq-100" et le fee de 0.0022 (0.22%)
fees = {
...
def preprocess_data(df, column_name, start_date, fee_rate):
    """
    Prépare les données : format datetime, trie par date croissante, limite à start_date, 
    applique les frais courants (en base annuelle, convertis en journaliers).
    """
    ...
    # Application des frais courants
...
# Important : Renommer la colonne correspondant au Nasdaq pour correspondre au code architecture → 'VL_Nasdaq100'
...
# Trier les dates et combler les valeurs manquantes
...
# On renomme "VL_Nasdaq" → "VL_Nasdaq100" pour être cohérent avec le code architecture
weights = {
...
def calculate_portfolio_value(df, weights):
    """
    Calcule la valeur totale du portefeuille en pondérant chaque VL par son poids,
    et en ramenant le tout sur une base initiale de 10 000€.
    """
    portfolio_value = sum(
...
def simulate_monthly_investment(df, monthly_investments):
    ...
    for investment in monthly_investments:
        ...
        for i, row in df.iterrows():
            if i % 21 == 0 and i != 0:  # Approx. 21 jours ouvrés par mois
...
def calculate_performance(df, results):
    """
    Calcule la performance avant/après impôts, en annualisé, etc.
    """
    performance_table = []
