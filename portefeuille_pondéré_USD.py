...
files = {
    ...
    "Nasdaq-100": "AMUNDI NASDAQ.xlsx",   # NOTE: Renommé pour être cohérent avec code architecture
...
# Frais courants (alignés sur code architecture + ancien code)
fees = {
...
def preprocess_data(df, column_name, start_date, fee_rate):
    """
    Prépare les données : format datetime, trie par date, applique un filtre start_date,
    puis applique des frais courants annuels convertis en journaliers.
    """
...
# Date de départ : identique à l'ancien code
start_date = pd.to_datetime("2017-10-09")

# Préparation des fichiers (avec cohérence de nommage)
...
df_nasdaq         = preprocess_data(df_nasdaq,        'VL_Nasdaq100',      start_date, fees["Nasdaq-100"])  # Changement clé : "Nasdaq-100"
...
# Pondérations (même logique "ancien" code, mais col. nasdaq = 'VL_Nasdaq100')
weights = {
    'VL_Treasury_Bond': 0.38, 
    'VL_Short_Duration': 0.12,
    'VL_SP500':         0.20,
    'VL_Nasdaq100':     0.20,   # IMPORTANT : 'VL_Nasdaq100' pour code archi.
    'VL_Small_Cap':     0.10,
}
...
def simulate_monthly_investment(df, monthly_investments):
    """
    Simule l'investissement mensuel en suivant la logique du code “ancien” :
    - On ajoute le montant (investment) au capital chaque fois i % 21 == 0 (sauf i=0).
    """
    results = {}
    for investment in monthly_investments:
        ...
        for i, row in df.iterrows():
            if i % 21 == 0 and i != 0:  # Comme dans l'ancien code
...
def calculate_performance(df, results):
    """
    Calcule la performance en reprenant la même logique que l'ancien code (10.59%).
    """
    performance_table = []
    for investment, data in results.items():
        total_return = (data['Portfolio'][-1] / data['Capital'][-1]) - 1
        # L'ancien code utilise (1 + total_return) ** (1 / (len(df) / 252)) - 1
...
# Si vous voulez tracer / afficher la performance :
# (Même logique que l'ancien code, base 10 000 + monthly invests.)
