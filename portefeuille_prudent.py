...
def simulate_portfolio(monthly_investment=100, initial_investment=10000):
    """
    Simule le portefeuille prudent (EUR) avec un investissement initial et des contributions mensuelles (DCA).
    ...
    # Définir les pondérations du portefeuille (prudent)
    weights = {
        'Euro Gov Bond': 0.50,
    ...
    def simulate_dca(df, monthly_investment):
        ...
        for i, row in df.iterrows():
            if i % 21 == 0 and i != 0:  # Approximation d'un mois
                total_invested += monthly_investment
            portfolio_value = row['Portfolio_Value'] * (total_invested / initial_investment)
    ...
    plt.title("Évolution du portefeuille (Prudent - DCA)")
    plt.xlabel("Date")
    plt.ylabel("Valeur (€)")
    plt.legend()
    plt.grid(True)
...
results, df_combined = simulate_portfolio(monthly_investment=100, initial_investment=10000)
